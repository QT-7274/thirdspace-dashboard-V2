...
点击 Quick 面板中的 `☐ 记TODO`，输入内容后 Enter 或点击「添加」。

弹框默认选中 `今日`，此时行为与旧版本一致：任务会追加到今日工作日志 `## 今日Todo` 章节。如当日工作日志不存在，自动创建。
...
任务池使用标准 Markdown checkbox，并自动附加范围标记：
...
## 长期
- [ ] 研究 Obsidian Tasks 插件兼容 #插件 #task/scope-longterm

## 指定日期
- [ ] 发布第一版 Todo 改造 #插件 #task/scope-custom 📅 2026-06-30
```

Dashboard 会在 `UPCOMING TASKS` 卡片中展示任务池里的未完成任务。点击任务行可打开任务池文件，点击 `加入今日` 会把该任务复制到当天 `TODAY'S TODOS`，不会删除任务池里的原任务。
